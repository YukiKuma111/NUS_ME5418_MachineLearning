...
class Group24(gym.Env, EzPickle):
    ...
    def _generate_terrain(self, hardcore):
        ...
        for i in range(TERRAIN_LENGTH):
            ...
            if state == GRASS and not oneshot:
                ...
                countery = self.np_random.integers(
                    VIEWPORT_H / SCALE - robot_height * 2 + HALF_HEIGHT_HULL / SCALE,
                ...
                poly = [
                    ...
                    (
                        x + (slope_width + robot_width * 1.5) * TERRAIN_STEP,
                        y + slope_height * TERRAIN_STEP,
                    ),
    ...
    def reset(
        ...
        *,
        ...
        lander = self.world.CreateDynamicBody(
            position=(init_x, init_y - (HALF_HEIGHT_HULL - HALF_HEIGHT_LANDER) / SCALE),
            fixtures=LANDER_FD,
        )
        ...
        for i in [-1, +1]:
            leg = self.world.CreateDynamicBody(
                position=(
                    init_x + i * (HALF_WIDTH_HULL / SCALE),
                    init_y - LEG_H / 2 - LEG_DOWN,
                ),
                angle=(i * 0.05),
                fixtures=LEG_FD,
            )
    ...
    def step(self, action: np.ndarray, fly_mode_flag=False):
        ...
        if fly_check:
            ...
            # impulse left
            impulse_left = (
                impulse_magnitude_left * tip[0],
                impulse_magnitude_left * tip[1],
            )
            ...
            # impulse right
            impulse_right = (
                impulse_magnitude_right * tip[0],
                impulse_magnitude_right * tip[1],
            )
            ...
            self.legs[0].ApplyLinearImpulse(scaled_impulse_left, impulse_pos_left, True)
            ...
            self.legs[2].ApplyLinearImpulse(
                scaled_impulse_right, impulse_pos_right, True
            )
        ...
        # Other modes
            ...
            self.joints[0].motorSpeed = float(
                SPEED_HIP * np.sign(action[0]) - leg_damping * rear_leg_speed
            )
            self.joints[0].maxMotorTorque = float(
                MOTORS_TORQUE * np.clip(np.abs(action[0]), 0, 1)
            )
            ...
            self.joints[2].motorSpeed = float(
                SPEED_HIP * np.sign(action[2]) - leg_damping * front_leg_speed
            )
            self.joints[2].maxMotorTorque = float(
                MOTORS_TORQUE * np.clip(np.abs(action[0]), 0, 1)
            )
        ...
        for a in action:
            ...
            if (
                ...
                reward -= (
                    ...
                    * ENGINE_POWER_FACTOR
                    * pow(blade_angular_speed_left, 2)
                    * pow(QUANTITY_OUTLINE_ADAPTOR, 2)
                )
