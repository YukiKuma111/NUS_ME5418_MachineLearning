...
class BipedalWalker(gym.Env, EzPickle):
        ...
        *,
        ...
        for i in [-1, +1]:
            ...
            wheel = self.world.CreateDynamicBody(
                position=(init_x + i * (HALF_WIDTH_HULL / SCALE), init_y - (HALF_HEIGHT_HULL / SCALE) - LEG_H - WHEEL_RADIUS / SCALE),
                angle=(i * 0.05),
                fixtures=WHEEL_FD,
            )
    ...
    def step(self, action: np.ndarray):
        ...
        if control_speed:
            ...
            self.joints[0].motorSpeed = float(SPEED_HIP * np.sign(action[0]) * (-1) - leg_damping * rear_leg_speed)
            ...
            self.joints[2].motorSpeed = float(SPEED_HIP * np.sign(action[0]) - leg_damping * front_leg_speed)
