...
class parallelEnv(VecEnv):
    def __init__(self, env_name,n=4, seed=None,spaces=None):
        ...
        if seed is not None:
            for i,e in enumerate(self.env_fns):
                # e.seed(i+seed)
                e.reset(seed=i+seed)    # Update here
                
        # self.max_steps = self.env_fns[0]._max_episode_steps        
        self.max_steps = self.env_fns[0].spec.max_episode_steps # Update here
    ...
    def reset(self):
        ...
        # Unpack the return value of recv() and extract only the observation part
